...
#define JULIA_CENTER_RE   0
#define JULIA_CENTER_IM   0
#define JULIA_ZOOM        4
 
/* ---------------------------------------------------------------
 * Mandelbrot auto-zoom tour (T while in Mandelbrot mode)
 * 3 phases: PAN → ZOOM_IN → ZOOM_OUT+PAN_BACK, loops forever
 * --------------------------------------------------------------- */
typedef enum {
    TOUR_IDLE = 0,
    TOUR_PANNING,
...
static const struct { double re, im; } ZOOM_TARGETS[] = {
    ...
    {-0.761574000000000, -0.084800000000000},
    {-1.250066000000000,  0.020000000000000},
    { 0.001643721000000,  0.822467633000000},
    {-0.170337000000000, -1.065156000000000},
    {-1.768778833000000, -0.001738996000000},
    {-0.748000000000000,  0.100000000000000},
};
...
/* ---------------------------------------------------------------
 * Julia c-parameter tour (T while in Julia mode)
 * Smoothly animates c through a list of visually interesting
 * Julia set parameters, dwelling briefly at each one.
 * --------------------------------------------------------------- */
#define JULIA_TOUR_MOVE_MS   3000  /* time to glide between two c values  */
#define JULIA_TOUR_DWELL_MS  1200  /* pause at each destination           */
 
static const struct { double re, im; } JULIA_C_TARGETS[] = {
    {-0.7000,  0.2700},  /* classic spiral                 */
    {-0.4000,  0.6000},  /* rabbit / Douady                */
    { 0.2850,  0.0100},  /* coral branches                 */
    {-0.7269,  0.1889},  /* Siegel disk                    */
    {-0.8000,  0.1560},  /* dendrite                       */
    {-0.1200, -0.7700},  /* San Marco dragon               */
    { 0.3000, -0.5000},  /* connected islands              */
    {-0.5400,  0.5400},  /* symmetric star                 */
    { 0.3700,  0.1000},  /* seahorse tail                  */
    {-0.1940,  0.6557},  /* feather                        */
    { 0,       0.8000},  /* cauliflower                    */
    {-0.6180,  0},       /* golden ratio / Fatou dust      */
};
#define NUM_JULIA_C_TARGETS (int)(sizeof(JULIA_C_TARGETS)/sizeof(JULIA_C_TARGETS[0]))
 
typedef enum {
    JULIA_TOUR_IDLE = 0,
    JULIA_TOUR_MOVING,   /* c gliding toward next target   */
    JULIA_TOUR_DWELLING  /* c arrived, short pause         */
} JuliaTourPhase;
...
int main(int argc, char *argv[]) {
    ...
    double    tour_target_re   = 0;
    double    tour_target_im   = 0;
    double    tour_deep_zoom   = 0;
    Uint32    tour_phase_start = 0;
    int       last_zoom_idx    = -1;
 
    /* ---- Julia tour ---- */
    JuliaTourPhase julia_tour       = JULIA_TOUR_IDLE;
    double         julia_tour_from_re = 0;
    double         julia_tour_from_im = 0;
    double         julia_tour_to_re   = 0;
    double         julia_tour_to_im   = 0;
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
                ...
                if (event.key.keysym.sym == SDLK_ESCAPE ||
                    ...
                    if (julia_mode) {
                        ...
                        if (julia_tour == JULIA_TOUR_IDLE) {
                            /* start: glide from current c to first random target */
            ...
            case SDL_MOUSEMOTION:
                ...
                if (is_panning) {
                    ...
                    /* only follow mouse when Julia tour is off */
                    double re_min, re_max, im_min, im_max;
                    calculate_boundaries(view.center_re, view.center_im, view.zoom,
                                         win_w, win_h, &re_min, &re_max, &im_min, &im_max);
                    julia_c.re = re_min + (double)mouse_x * (re_max - re_min) / win_w;
        ...
        /* ================================================================
         * Julia c-parameter tour
         * Smoothly glides c between interesting Julia set parameters.
         * MOVING  : c interpolates from_c → to_c with smoothstep easing
         * DWELLING: c stays at to_c for a short pause, then picks next
         * ================================================================ */
        if (julia_tour != JULIA_TOUR_IDLE) {
            Uint32 now = SDL_GetTicks();
            ...
            if (julia_tour == JULIA_TOUR_MOVING) {
                ...
                if ((double)(now - julia_tour_start) >= JULIA_TOUR_DWELL_MS) {
                    /* pick next target and start moving */
                    julia_tour_from_re = julia_c.re;
                    julia_tour_from_im = julia_c.im;
