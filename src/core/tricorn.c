/* tricorn.c
 *
 * mathematical kernel for computing the tricorn (mandelbar) fractal.
 * negates the imaginary part of the complex number during iteration.
 */
...
// scalar tricorn path: single pixel, no cardioid rejection (tricorn has none)
double tricorn_check(complex_t c, int max_iterations) {
    complex_t z = {0, 0};
    int iterations = 0;
    const double escape_radius_sq = ESCAPE_RADIUS * ESCAPE_RADIUS;

    /* tricorn formula: z_{n+1} = conj(z_n)^2 + c (negates z.im each step)     * pre-computing zre2
     * and zim2 saves one multiplication per iteration. */
...
void tricorn_check_avx512(__m512d cre, __m512d cim, int max_iterations, double* results) {
    ...
    for (int i = 0; i < max_iterations; i++) {
        ...
        zim =
            _mm512_add_pd(_mm512_sub_pd(_mm512_setzero_pd(), _mm512_add_pd(zre_zim, zre_zim)), cim);
...
void tricorn_check_neon(float64x2_t cre, float64x2_t cim, int max_iterations, double* results) {
    ...
    for (int i = 0; i < max_iterations; i++) {
        ...
        zim = vaddq_f64(vsubq_f64(vdupq_n_f64(0.0), vaddq_f64(zre_zim, zre_zim)), cim);
...
#ifdef USE_SIMD_F128
/* high-precision 128-bit tricorn path (double-double):
...
#ifdef __AVX2__
/* avx2 high-precision 128-bit tricorn path:
