...
void tricorn_check_avx512(__m512d cre, __m512d cim, int max_iterations, double* results) {
    ...
    for (int i = 0; i < max_iterations; i++) {
        ...
        zim = _mm512_add_pd(_mm512_sub_pd(_mm512_setzero_pd(), _mm512_add_pd(zre_zim, zre_zim)), cim);
...
void tricorn_check_f128x4(simd_f128x4 cre, simd_f128x4 cim, int max_iterations,
                             double* results) {
