...
void hud_render_video_studio(struct ImFont* custom_font, AppCommonState* state, int win_w,
    ...
    igSetNextWindowPos((ImVec2){win_w * 0.5f, win_h * 0.5f}, ImGuiCond_Always,
                       (ImVec2){0.5f, 0.5f});
            ...
            if (igBeginTabBar("VideoStudioTabs", ImGuiTabBarFlags_None)) {
                ...
                if (igBeginTabItem("Output", NULL, ImGuiTabItemFlags_None)) {
                    ...
                    for (int i = 1; i < res_count; i++) {
                        if (state->video_settings.res_w == presets[i].w &&
                            state->video_settings.res_h == presets[i].h) {
                            current_preset = i;
                            break;
                        }
                    }

                    if (igCombo_Str_arr("Resolution Preset", &current_preset, res_names, res_count,
                                        -1)) {
                        if (current_preset > 0) {
    ...
    for (int i = 0; i < 5; i++) {
        if (state->notifications[i].active) {
            ...
            if (elapsed > 3.0f) {
                ...
                igSetNextWindowPos((ImVec2){20.0f, 20.0f + i * 40.0f}, ImGuiCond_Always,
                                   (ImVec2){0.0f, 0.0f});
