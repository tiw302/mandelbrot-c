...
layout(binding=0) uniform params_t {
    ...
    vec2 u_julia_c_lo;
    float u_zoom;
    float u_zoom_lo;  // low part of zoom for Dekker hi-lo d0 calculation
    float u_iters;
    float u_aspect;
    vec2 u_ref_offset;
    float u_fractal_type;  // 0=mandelbrot, 1=julia, 2=burning_ship, 3=tricorn, 4=celtic, 5=buffalo
    float u_palette;
    float u_high_precision;
    float u_use_perturbation;
    float u_orbit_len;
};
layout(binding=0) uniform texture2D u_orbit;
...
void main() {
  ...
  if (u_use_perturbation > 0.5 && u_fractal_type < 0.5) {
    ...
    for (; i < 2000; i++) {
      ...
      vec4 Zn_data = texture(sampler2D(u_orbit, u_orbit_smp), vec2((float(i) + 0.5) / 10000.0, 0.5));
...
@program desktop_gpu dg_vs dg_fs_gpu
